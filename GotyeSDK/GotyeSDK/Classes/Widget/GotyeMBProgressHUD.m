...
@implementation GotyeMBProgressHUD {
	BOOL useAnimation;
...
+ (GotyeMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	GotyeMBProgressHUD *hud = [[GotyeMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	...
	for (GotyeMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (GotyeMBProgressHUD *)HUDForView:(UIView *)view {
	GotyeMBProgressHUD *hud = nil;
	NSArray *subviews = view.subviews;
	Class hudClass = [GotyeMBProgressHUD class];
	for (UIView *view in subviews) {
		if ([view isKindOfClass:hudClass]) {
			hud = (GotyeMBProgressHUD *)view;
		}
	}
	return hud;
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = GotyeMBProgressHUDAnimationFade;
		self.mode = GotyeMBProgressHUDModeIndeterminate;
		self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
	self.alpha = 0.0f;
	if (animated && animationType == GotyeMBProgressHUDAnimationZoom) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == GotyeMBProgressHUDAnimationZoom) {
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[GotyeMBRoundProgressView class]];
	
	if (mode == GotyeMBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
	...
	else if (mode == GotyeMBProgressHUDModeDeterminate || mode == GotyeMBProgressHUDModeAnnularDeterminate) {
		if (!isRoundIndicator) {
			// Update to determinante indicator
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[GotyeMBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == GotyeMBProgressHUDModeAnnularDeterminate) {
			[(GotyeMBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
	else if (mode == GotyeMBProgressHUDModeCustomView && customView != indicator) {
		// Update custom view indicator
		[indicator removeFromSuperview];
		self.indicator = customView;
		[self addSubview:indicator];
	} else if (mode == GotyeMBProgressHUDModeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
	}
}
...
@implementation GotyeMBRoundProgressView {
	float _progress;
	BOOL _annular;
}
